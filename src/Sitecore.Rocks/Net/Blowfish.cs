...
namespace Sitecore.Rocks.Net
{
    public class BlowFish
    {
        ...
        public BlowFish([NotNull] string hexKey)
        {
            randomSource = new RNGCryptoServiceProvider();
            SetupKey(HexToByte(hexKey));
        }

        public BlowFish([NotNull] byte[] cipherKey)
        {
            randomSource = new RNGCryptoServiceProvider();
            SetupKey(cipherKey);
        }
        ...
        {
            get { return "DBF92748F42842C4A25D6F4AFA56712444EA4FD8754B4A79AA7E996C4BB66DA4"; }
        }

        public string Decrypt_CBC([NotNull] string ct)
        {
            IV = HexToByte(ct.Substring(0, 16));
            return Encoding.ASCII.GetString(Decrypt_CBC(HexToByte(ct.Substring(16)))).Replace("\0", string.Empty);
        }

        public byte[] Decrypt_CBC([NotNull] byte[] ct)
        {
            return Crypt_CBC(ct, true);
        }

        public string Decrypt_ECB([NotNull] string ct)
        {
            return Encoding.ASCII.GetString(Decrypt_ECB(HexToByte(ct))).Replace("\0", string.Empty);
        }

        public byte[] Decrypt_ECB([NotNull] byte[] ct)
        {
            return Crypt_ECB(ct, true);
        }

        public string Encrypt_CBC([NotNull] string pt)
        {
            if (!ivset)
            {
                SetRandomIV();
        ...
        public byte[] Encrypt_CBC([NotNull] byte[] pt)
        {
            return Crypt_CBC(pt, false);
        }

        public string Encrypt_ECB([NotNull] string pt)
        {
            return ByteToHex(Encrypt_ECB(Encoding.ASCII.GetBytes(pt)));
        }

        public byte[] Encrypt_ECB([NotNull] byte[] pt)
        {
        ...
        private void BlockDecrypt([NotNull] ref byte[] block)
        {
            SetBlock(block);
            decipher();
            GetBlock(ref block);
        ...
        private void BlockEncrypt([NotNull] ref byte[] block)
        {
            SetBlock(block);
            encipher();
            GetBlock(ref block);
        ...
        private string ByteToHex([NotNull] byte[] bytes)
        {
        ...
        [NotNull]
        private byte[] Crypt_CBC([NotNull] byte[] text, bool decrypt)
        {
            if (!ivset)
            {
        ...
        [NotNull]
        private byte[] Crypt_ECB([NotNull] byte[] text, bool decrypt)
        {
            var paddedLen = text.Length % 8 == 0 ? text.Length : text.Length + 8 - text.Length % 8;
            var plainText = new byte[paddedLen];
            Buffer.BlockCopy(text, 0, plainText, 0, text.Length);
        ...
        private void GetBlock([NotNull] ref byte[] block)
        {
        ...
        [NotNull]
        private byte[] HexToByte([NotNull] string hex)
        {
        ...
        private void SetBlock([NotNull] byte[] block)
        {
            var block1 = new byte[4];
            var block2 = new byte[4];
        ...
        private void SetupKey([NotNull] byte[] cipherKey)
        {
            bfp = SetupP();
        ...
        private void XorBlock([NotNull] ref byte[] block, [NotNull] byte[] iv)
        {
        ...
        {
            var x1 = (s0[wordByte0(b)] + s1[wordByte1(b)]) ^ s2[wordByte2(b)];
            var x2 = x1 + s3[wordByte3(b)];
            var x3 = x2 ^ bfp[n];
            return x3 ^ a;
        }
