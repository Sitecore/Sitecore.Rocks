...
{
    public class ContentTreeContext : ICommandContext, IItemSelectionContext, IDatabaseSelectionContext, ISiteSelectionContext, IFileSelectionContext, IDeleteItemContext
    {
        public ContentTreeContext([NotNull] ItemTreeView contentTree, [NotNull] IEnumerable<BaseTreeViewItem> selectedItems)
        {
        ...
        IEnumerable<IHasFileUri> IFileSelectionContext.Files
        {
            get
            {
                foreach (var item in SelectedItems.OfType<IHasFileUri>())
                {
                    yield return item;
                }
            }
        ...
        Site ISiteSelectionContext.Site
        {
            get
            {
                ...
                foreach (var baseTreeViewItem in SelectedItems)
                {
                    ...
                    var file = baseTreeViewItem as IHasFileUri;
                    if (file != null)
                    {
                        s = file.FileUri.Site;
                    }
