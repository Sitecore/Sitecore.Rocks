...
using Sitecore.Rocks.UI.Management.ManagementItems.SiteViewers;
using Sitecore.Rocks.UI.StartPage.Commands;

namespace Sitecore.Rocks.UI.StartPage.Tabs.GetStarted.Configuring.WebConfig
{
    [Command(Submenu = ToolsSubmenu.Name), StartPageCommand("Explore and create new sites", StartPageWebConfigGroup.Name, 1000), Feature(FeatureNames.Management)]
    public class ExploreSites : StartPageCommandBase
    {
        public ExploreSites()
        {
            Text = "Sites";
        ...
        private void Execute([NotNull] Site site)
        {
            ...
            AppHost.Windows.Factory.OpenManagementViewer(site.Name, new SiteManagementContext(site), SiteViewer.ItemName);
        }
    }
}
